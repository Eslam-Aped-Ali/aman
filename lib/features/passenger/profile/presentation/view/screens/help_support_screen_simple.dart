...
class _ContactBottomSheet extends StatelessWidget {
  ...
  void _openEmail(BuildContext context) async {
    try {
      final Uri emailUri = Uri(
        scheme: 'mailto',
        path: 'support@aman-transport.com',
        queryParameters: {
          'subject': 'Support Request - Aman App',
        },
      );
      ...
      if (await canLaunchUrl(emailUri)) {
        ...
        await Clipboard.setData(
            const ClipboardData(text: 'support@aman-transport.com'));
        FlashHelper.showToast('Email copied to clipboard');
        Navigator.of(context).pop();
      }
    } catch (e) {
      FlashHelper.showToast('Failed to open email app',
          type: MessageTypeTost.fail);
    }
  }
  ...
  void _openPhone(BuildContext context) async {
    try {
      const String phoneNumber =
          '+1-800-AMAN-HELP'; // Replace with actual support phone
      ...
      if (await canLaunchUrl(phoneUri)) {
        ...
        FlashHelper.showToast('Phone number copied to clipboard');
        Navigator.of(context).pop();
      }
    } catch (e) {
      FlashHelper.showToast('Failed to open phone app',
          type: MessageTypeTost.fail);
    }
  }
}
  ...
  void _openWebsite(BuildContext context) async {
    try {
      const String websiteUrl =
          'https://www.aman-transport.com'; // Replace with actual website
      ...
      if (await canLaunchUrl(uri)) {
        ...
        await Clipboard.setData(const ClipboardData(text: websiteUrl));
        FlashHelper.showToast('Website URL copied to clipboard');
        Navigator.of(context).pop();
      }
    } catch (e) {
      FlashHelper.showToast('Failed to open website',
          type: MessageTypeTost.fail);
    }
  }
}
