...
class CrashReporter {
  ...
  static void addBreadcrumb(String message, [Map<String, dynamic>? data]) {
    ...
    if (kDebugMode) {
      Console.printDebug(
          '🍞 Breadcrumb: $message ${data != null ? jsonEncode(data) : ''}');
    }
  }
  ...
  static Future<bool> _sendReportToServer(CrashReport report) async {
    // Implementation for crash report server endpoint
    try {
      if (kDebugMode) {
        Console.printWarning(
            'Crash report (not sent to server in debug mode):');
        Console.printDebug(jsonEncode(report.toJson()));
        return true; // Simulate success in debug mode
      }

      // In production, you would send to your crash reporting service
      // Examples: Firebase Crashlytics, Sentry, Bugsnag, or custom endpoint
      /*
      final response = await dio.post(
        'https://your-crash-api.com/reports',
        data: report.toJson(),
        options: Options(
      ...
      */
      ...
      // For now, just log locally and return true
      Console.printInfo('Crash report would be sent to server in production');
      return true;
    } catch (e) {
      Console.printError('Failed to send crash report: $e');
      return false;
    }
  }
  ...
  static Future<void> _storeReportLocally(CrashReport report) async {
    ...
    try {
      ...
      final file = File(
          '${dir.path}/crashes/crash_${DateTime.now().millisecondsSinceEpoch}.json');
