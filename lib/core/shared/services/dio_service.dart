...
import 'package:Aman/core/shared/costants/app_constants.dart';
  ...
  void _setupDio() {
    ...
    (_dio.httpClientAdapter as IOHttpClientAdapter).createHttpClient = () {
      ...
      client.badCertificateCallback =
          (cert, host, port) => true; // For dev only
      return client;
    };
    ...
    try {
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (T == String && responseData is String) {
          // For string responses, return as is
          ...
          return ApiResponse<T>.success(
            data: responseData as T?,
            message: responseData is String ? responseData : 'Success',
          );
        } else {
          // For other types (expecting JSON)
          String? message;
...
class _RetryInterceptor extends Interceptor {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    ...
    if (_shouldRetry(err) && retryCount < maxRetries) {
      ...
      try {
        final response =
            await DioService.instance._dio.fetch(err.requestOptions);
